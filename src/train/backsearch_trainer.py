...
class BackTrainer(object):
    ...
    def find_fix(self, preds, gts, all_probs, gt_exprs, num_lists):
        ...
        #         expr_str = [ for x in gt_expr]
                ...
                if (etree.res()[0] -  gt) < 1e-2:
                    ...
                    #     print(None)
                    # gt_expr = gt_expr[:l]
                    # gt_prob = np.sum([all_prob[i, gt_expr[i]] for i in range(l)])
    ...
    def compute_gen_ans(self, seq_var, num_list, post_flag):
        ...
        equ_list = self.get_new_tempalte(seq_var, num_list)
    ...
    def _train_batch(self, input_variables, input_lengths,target_variables, target_lengths, model,\
        ...
        for i in range(batch_size):
            ...
            for j in range(len(decoder_outputs)): 
                ...
                all_probs_once.append(decoder_outputs[j][i][2:].cpu().data.numpy())
            ...
            all_probs.append(all_probs_once)
        ...
        for i in range(batch_size):
            ...
            gen_answers.append(gen_ans)
            gt_equations.append(gt_equ)
            gen_equations.append(gen_equ)
        
        gen_answers = np.array(gen_answers)
        gen_equations = np.array (gen_equations)
        gt_equations = np.array (gt_equations)
        solutions = np.array (solutions)
        
        fix_list = self.find_fix(gen_equations, solutions, all_probs, gt_equations, num_list)
        
        self.loss.reset()
        for step, step_output in enumerate(decoder_outputs):
            # cuda step_output = step_output.cuda()
            fix_step = torch.ones(batch_size, dtype = torch.long)
            target = target_variables[:, step]

            if self.cuda_use:
                step_output = step_output.cuda()
                fix_step = fix_step.cuda()

            for i in range(batch_size):
                if len(fix_list[i]):
                    if step < len(fix_list[i]):
                        fix_step[i] = fix_list[i][step]
    ...
    def train(self, model, data_loader, batch_size, n_epoch, template_flag, \
        ...
        if resume:
            ...
            if optimizer is None:
                optimizer = Optimizer(optim.Adam(model.parameters(), lr=1e-4), max_grad_norm=0)
