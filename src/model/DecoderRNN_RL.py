...
class DecoderRNN_RL(BaseRNN):
    def __init__(self, vocab_size, class_size, embed_model=None, emb_size=100, hidden_size=128, \
        ...
        super(DecoderRNN_RL, self).__init__(vocab_size, emb_size, hidden_size,
    ...
    def forward_normal_no_teacher(self, decoder_input, decoder_init_hidden, encoder_outputs,\
        ...
        mask_var = torch.ones((batch_size, len(self.class_list)))
        filters_var = []
        for k,v in self.class_dict.items():
            if 'PI' == k or k.isdigit():
                filters_var.append(v)
        filters_var = np.array(filters_var)
        mask_var[:, filters_var] = 0.5
